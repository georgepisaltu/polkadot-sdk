...
/// The forwarded asset's id from this chain's perspective, as the union instance names it.
...
fn forward_asset_works() {
	new_test_ext().execute_with(|| {
		create_asset(true);
		assert_ok!(Forwarder::forward_asset(RuntimeOrigin::signed(BOB), ASSET_ID));
...
fn forward_asset_fails_for_unknown_asset() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			Forwarder::forward_asset(RuntimeOrigin::signed(BOB), ASSET_ID),
...
fn forward_asset_fails_when_already_forwarded() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Forwarder::forward_asset(RuntimeOrigin::signed(BOB), ASSET_ID));
		assert_noop!(
			Forwarder::forward_asset(RuntimeOrigin::signed(ALICE), ASSET_ID),
...
fn forward_asset_fails_without_deposit_funds() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Forwarder::forward_asset(RuntimeOrigin::signed(poor), ASSET_ID),
...
fn sync_asset_status_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Forwarder::sync_asset_status(RuntimeOrigin::signed(ALICE), ASSET_ID));
...
fn sync_asset_status_rejects_unchanged_status() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Forwarder::sync_asset_status(RuntimeOrigin::signed(ALICE), ASSET_ID),
...
fn sync_asset_status_fails_when_not_forwarded() {
	new_test_ext().execute_with(|| {
		create_asset(true);
		assert_noop!(
			Forwarder::sync_asset_status(RuntimeOrigin::signed(BOB), ASSET_ID),
...
fn remove_forwarded_asset_works() {
	new_test_ext().execute_with(|| {
		create_asset(true);
		assert_ok!(Forwarder::forward_asset(RuntimeOrigin::signed(BOB), ASSET_ID));
		assert_ok!(Forwarder::remove_forwarded_asset(RuntimeOrigin::root(), ASSET_ID));
		...
		assert_ok!(Forwarder::forward_asset(RuntimeOrigin::signed(ALICE), ASSET_ID));
		assert_eq!(
...
fn remove_forwarded_asset_fails_for_non_manager() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Forwarder::forward_asset(RuntimeOrigin::signed(BOB), ASSET_ID));
		assert_noop!(
			Forwarder::remove_forwarded_asset(RuntimeOrigin::signed(ALICE), ASSET_ID),
...
fn remove_forwarded_asset_fails_when_not_forwarded() {
	new_test_ext().execute_with(|| {
		create_asset(true);
		assert_noop!(
			Forwarder::remove_forwarded_asset(RuntimeOrigin::root(), ASSET_ID),
			Error::<Test>::NotForwarded
		);
	});
}
...
#[test]
fn union_forwarder_names_local_asset_by_its_location() {
	new_test_ext().execute_with(|| {
		create_asset(true);
		assert_ok!(UnionForwarder::forward_asset(
			RuntimeOrigin::signed(BOB),
			local_asset_location()
		));

		// The union resolves the location to the `Assets` entry, so the status comes from there
		// and the replica gets the same id the `u32`-keyed instance would produce.
		let calls = sent_transacts();
		assert_eq!(calls.len(), 1);
		assert!(matches!(
			&calls[0],
			RuntimeCall::DestAssets(pallet_assets::Call::force_create {
				...
				min_balance: MIN_BALANCE,
				..
			}) if *id == remote_asset_id()
		));
		let info = ForwardedAssets::<Test, UnionForwarderInstance>::get(local_asset_location())
			.expect("asset is recorded");
		assert_eq!(info.min_balance, MIN_BALANCE);
		assert!(info.is_sufficient);
	});
}
...
#[test]
fn union_forwarder_keeps_foreign_asset_location() {
	new_test_ext().execute_with(|| {
		...
		assert!(matches!(
			...
			RuntimeCall::DestAssets(pallet_assets::Call::force_create {
				...
				min_balance,
				..
			}) if *id == foreign_asset_location() && *min_balance == MIN_BALANCE * 3
		));

		// The owner is derived from the union instance's own pallet index.
		let owner = <Test as crate::Config>::DestinationAccountOf::convert_location(
			&Location::new(1, [Parachain(1500), PalletInstance(38)]),
		)
		.expect("forwarder location converts to an account");
		assert!(matches!(
			&calls[0],
			RuntimeCall::DestAssets(pallet_assets::Call::force_create { owner: o, .. })
				if *o == owner.clone().into()
		));
	});
}

#[test]
fn union_forwarder_rejects_unknown_locations() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			UnionForwarder::forward_asset(RuntimeOrigin::signed(BOB), foreign_asset_location()),
			Error::<Test, UnionForwarderInstance>::UnknownAsset
		);
		assert_noop!(
			UnionForwarder::forward_asset(RuntimeOrigin::signed(BOB), local_asset_location()),
			Error::<Test, UnionForwarderInstance>::UnknownAsset
		);
	});
}
